...
contract DebtPreviewerTest is ForkTest {
  ...
  function testMinDepositCrossAsset() external {
    debtManager.leverage(marketUSDC, 10_000e6, 4e18);
    Leverage memory leverage = debtPreviewer.leverage(marketWETH, marketUSDC, address(this), 1.05e18);
    ...
    debtManager.crossLeverage(marketWETH, marketUSDC, 500, leverage.minDeposit, limit.ratio, MAX_SQRT_RATIO - 1);
    assertApproxEqAbs(limit.deposit, marketWETH.maxWithdraw(address(this)), 2);
    assertApproxEqAbs(limit.borrow, floatingBorrowAssets(marketUSDC, address(this)), 2);
  }
  ...
  function testLeverageRatesCrossAsset() external {
    ...
    emit log_named_decimal_uint("rates.deposit  ", rates.deposit, 18);
    emit log_named_decimal_uint("rates.borrow   ", rates.borrow, 18);
    emit log_named_decimal_uint("rates.native   ", rates.native, 18);
    for (uint256 i = 0; i < rates.rewards.length; ++i) {
      emit log_named_address("rewards.asset ", rates.rewards[i].asset);
      emit log_named_string("rewards.symbol ", rates.rewards[i].assetSymbol);
      emit log_named_string("rewards.assetName ", rates.rewards[i].assetName);
      emit log_named_decimal_uint("rewards.borrow ", rates.rewards[i].borrow, 18);
      emit log_named_decimal_uint("rewards.deposit ", rates.rewards[i].deposit, 18);
    }
  }
