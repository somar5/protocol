...
import { PriceFeedPool, IPriceFeed, IPool } from "../../contracts/PriceFeedPool.sol";
...
contract PriceFeedPoolTest is Test {
  ...
  function testPriceFeedPoolWithDifferentDecimals() external {
    priceFeedPool = new PriceFeedPool(
      IPool(address(new MockPool(new MockERC20("WETH", "WETH", 18), new MockERC20("EXA", "EXA", 8), 100e18, 500e8))),
...
contract MockPool {
  ...
  constructor(MockERC20 token0_, MockERC20 token1_, uint256 reserve0_, uint256 reserve1_) {
    token0 = token0_;
    token1 = token1_;
    reserve0 = reserve0_;
    reserve1 = reserve1_;
